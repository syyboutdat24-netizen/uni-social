...
export default async function UserProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: profile } = await supabase
    .from("profiles")
    .select("id, full_name, avatar_url, bio, role, badge_role")
    .eq("id", id)
    .maybeSingle();
  ...
  const connStatus = !myConn ? "none"
    : myConn.status === "accepted" ? "connected"
    : myConn.sender_id === user.id ? "pending_sent"
    : "pending_received";
    ...
    <div className="min-h-screen app-bg app-text">
      ...
      <div className="max-w-3xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="app-surface rounded-2xl border app-border overflow-hidden">
          <div className="h-32 bg-gradient-to-r from-indigo-900/50 to-zinc-800" />
          <div className="px-6 pb-6">
            <div className="flex items-end justify-between -mt-12 mb-4">
              ...
              <div className="flex gap-2">
                {isOwnProfile && (
                  <a href="/profile" className="app-input-bg hover:opacity-80 app-text text-sm px-4 py-2 rounded-lg border app-border">
                    Edit Profile
                  </a>
                )}
                {!isOwnProfile && connStatus === "none" && (
                  <form method="POST" action={`/api/connections/send?to=${id}`}>
                    <button type="submit" className="bg-indigo-600 hover:bg-indigo-700 text-white text-sm px-4 py-2 rounded-lg">
                      Connect
                    </button>
                  </form>
                )}
                ...
                {!isOwnProfile && connStatus === "connected" && (
                  <a href={`/messages/${id}`} className="app-input-bg hover:opacity-80 app-text text-sm px-4 py-2 rounded-lg border app-border">
                    Message
                  </a>
                )}
              </div>
            ...
            {profile.role && <p className="app-text-muted text-sm mb-3">{profile.role}</p>}
